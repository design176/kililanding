...
**Kili Deck** (`/files/kili_deck`, `src/components/files/deck/`) is a client-side slide viewer - `KiliDeck.tsx` owns slide index/keyboard nav, `Slides.tsx` holds the six slide components, `CarouselDots` is the dot pagination with an auto-advancing fill-bar, `StatTile` is the KPI-tile-style stat box. `Badge` (`src/components/ui/Badge.tsx`, tones `neutral`/`brand`/`danger`) is shared with the `/files` index. All of it takes design cues - Badge, KPI Tile, Card, Carousel Dots - from the atoms/molecules documented at `https://kili-app-demo-test-not-public.vercel.app/design/components`, reimplemented against this repo's own `globals.css` tokens rather than that app's palette.
